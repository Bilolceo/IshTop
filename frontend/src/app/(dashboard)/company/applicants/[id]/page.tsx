...
const statusConfig: Record<KnownApplicationStatus, { label: string; color: string; icon: any }> = {
  ...
  interview: { label: "Intervyu", color: "bg-brand-100 text-brand-700", icon: User },
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              {application.status === "interview" && (
                <Badge className="bg-brand-100 text-brand-700">Intervyu faol</Badge>
              )}
            </div>
                    ...
                    <ul className="space-y-1">
                      {application.match_breakdown.matched_skills.slice(0, 10).map((skill) => (
                        <li key={skill} className="flex items-start gap-2 text-sm text-surface-700 dark:text-surface-300">
                          <Check className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-brand-600" />
                          <span>{skill}</span>
                        </li>
                      ))}
          ...
          <motion.div
            ...
            className="overflow-hidden rounded-2xl border border-brand-200 bg-gradient-to-br from-brand-50 to-white p-5 shadow-sm dark:border-brand-500/30 dark:from-brand-500/10 dark:to-surface-800"
          >
            <div className="mb-4 flex items-center gap-2">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-brand-500 to-violet-600 text-white shadow-md">
            ...
            <div className="space-y-3">
              ...
              {aiSummary && (
                <div className="rounded-xl border border-brand-100 bg-white p-4 dark:border-brand-500/20 dark:bg-surface-900/50">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      {aiSummary.fit_score !== null && (
                        <span className="rounded-md bg-brand-100 px-2 py-0.5 text-xs font-bold text-brand-700 dark:bg-brand-500/20 dark:text-brand-300">
                          {aiSummary.fit_score}%
                        </span>
                      )}
                      <span
            ...
            <div className="mt-5 space-y-2">
              ...
              {aiQuestions && aiQuestions.length > 0 && (
                <ol className="space-y-2 rounded-xl border border-brand-100 bg-white p-4 dark:border-brand-500/20 dark:bg-surface-900/50">
                  {aiQuestions.map((q, i) => (
                    <li key={i} className="flex gap-3 text-sm">
                      <span className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-brand-100 text-xs font-bold text-brand-700 dark:bg-brand-500/20 dark:text-brand-300">
                        {i + 1}
                      </span>
            ...
            <div className="mt-5 space-y-2">
              ...
              {aiEmail && (
                <div className="space-y-2 rounded-xl border border-brand-100 bg-white p-4 dark:border-brand-500/20 dark:bg-surface-900/50">
                  <div>
              ...
              <div className="mt-5 space-y-2 border-t border-surface-200 pt-4 dark:border-surface-700">
                ...
                {scorecards.map((s) => (
                  ...
                  >
                    ...
                    <span className="rounded-md bg-brand-100 px-2 py-1 text-sm font-bold text-brand-700 dark:bg-brand-500/20 dark:text-brand-300">
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {resume.content.skills.technical.map((s) => (
                      <span key={s} className="rounded-full bg-brand-100 px-2.5 py-0.5 text-xs font-medium text-brand-700">
                        {s}
                      </span>
                    ))}
                  </div>
