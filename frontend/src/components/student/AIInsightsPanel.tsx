...
function buildInsightLines(input: InsightsInput): string[] {
  const name = (input.fullName?.split(" ")[0] || "siz").trim();
  const lines: string[] = [];
...
export function AIInsightsPanel({ input }: { input: InsightsInput }) {
  ...
  const script = useMemo(() => buildInsightLines(input), [
    input.fullName,
    input.resumes,
    input.applications,
    input.interviews,
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-black/40 p-5 font-mono text-sm shadow-2xl shadow-violet-500/10 backdrop-blur-md dark:bg-black/40 sm:p-6">
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            aria-label="Tahlilni qayta yuritish"
