      ...
      <div className="section-shell py-16 sm:py-24">
        ...
        <div className="mt-8 flex flex-wrap gap-2">
          <span className="chip !border-white/10 !bg-white/[0.04] !text-white/80">
            <CheckCircle2 className="h-3 w-3 text-brand-300" /> 60+ components
          </span>
          <span className="chip !border-white/10 !bg-white/[0.04] !text-white/80">
            <CheckCircle2 className="h-3 w-3 text-brand-300" /> W3C tokens
          </span>
          <span className="chip !border-white/10 !bg-white/[0.04] !text-white/80">
            <CheckCircle2 className="h-3 w-3 text-brand-300" /> WCAG AA
          </span>
          <span className="chip !border-white/10 !bg-white/[0.04] !text-white/80">
            <CheckCircle2 className="h-3 w-3 text-brand-300" /> Reduce-motion safe
          </span>
        </div>
      ...
      <ul className="space-y-0.5">
        {SECTIONS.map((s) => (
          <li key={s.id}>
            <a
              href={`#${s.id}`}
              className="focus-ring block rounded-xl px-3 py-1.5 text-sm font-medium text-surface-700 hover:bg-surface-100 hover:text-brand-600 dark:text-white/70 dark:hover:bg-white/[0.04] dark:hover:text-brand-300"
            >
              {s.label}
            </a>
          </li>
  ...
  return (
    <section id={id} aria-labelledby={`${id}-h`} className="scroll-mt-20">
      <header className="border-b border-surface-200/70 pb-4 dark:border-white/[0.06]">
        <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-brand-500 dark:text-brand-300">
          /{id}
        </p>
...
function Colors() {
  const swatches = [
    ...
    { name: "aurora/violet", hex: "#6F9BF0", role: "Primary brand" },
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className="card-aurora overflow-hidden">
          <div
            ...
            style={{
              background:
                "linear-gradient(120deg, #6F9BF0 0%, #5B8CFF 60%, #22D3EE 110%)",
            }}
          />
          <div className="p-4">
        ...
        <div className="card-aurora overflow-hidden">
          <div
            ...
            style={{
              background:
                "conic-gradient(from 120deg at 50% 50%, #6F9BF0, #22D3EE, #F5B544, #6F9BF0)",
            }}
          />
          <div className="p-4">
        ...
        <div className="space-y-3">
          {scale.map((n) => (
            <div key={n} className="flex items-center gap-4">
              ...
              <span
                aria-hidden
                className="block h-3 rounded-full bg-gradient-to-r from-brand-500 to-cyan-400"
                style={{ width: n * 4 }}
              />
            </div>
          ))}
        ...
        <div className="flex flex-wrap items-end gap-5">
          {radii.map((r) => (
            <div key={r.name} className="text-center">
              <div className={`h-20 w-20 ${r.className} bg-gradient-to-br from-brand-500 to-cyan-400`} />
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {[
            ...
            <button
              key={m.c}
              type="button"
              className={`group h-24 rounded-2xl bg-gradient-to-br from-brand-500 to-cyan-400 text-sm font-semibold text-white transition-transform ${m.c} hover:scale-105 hover:rotate-1`}
            >
    ...
    <Section id="badges" title="Badges & chips" description="Use for tags, status, and meta information. Never decoration.">
      <PreviewBox>
        <div className="flex flex-wrap gap-2">
          ...
          <span className="chip">
            <CheckCircle2 className="h-3 w-3 text-brand-500" /> Verified
          </span>
          <span className="chip">
            <Star className="h-3 w-3 text-amber-500" /> 4.9 rating
          </span>
          <span className="rounded-full bg-brand-500/15 px-2.5 py-1 text-xs font-semibold text-brand-700 dark:text-brand-300">
            94% match
          </span>
        </div>
      </PreviewBox>
...
function MobilePatterns() {
  ...
  const slides = [
    { title: "AI Resume", body: "Bullet pointlar bir tugma bilan kuchayadi", color: "#6F9BF0" },
...
tokens.color.aurora.violet.$value;   // "#6F9BF0"
